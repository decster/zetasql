...
class ASTFilterFieldsExpression final : public ASTExpression {
 public:
  ...
  const ASTFilterFieldsArg* arguments(int i) const { return arguments_[i]; }
...
class ASTCreatePrivilegeRestrictionStatement final : public ASTCreateStatement {
 public:
  ...
  const ASTPrivileges* column_privilege_list() const { return column_privilege_list_; }
 ...
 private:
  void InitFields() final {
    FieldLoader fl(this);
    fl.AddRequired(&column_privilege_list_);
  ...
  bool is_or_replace_ = false;
  bool is_if_not_exists_ = false;
  const ASTPrivileges* column_privilege_list_ = nullptr;
